		...
		<table class="MainArea_Content" align="center" cellspacing="0"
			...
			<tbody id="TableData">
				<c:forEach items="${requestScope.list}" var="o">
					<tr height="60">
						<td>${o.id}</td>
						<c:forEach  items="${requestScope.table}" var="table">
						...
						<td><a href="${pageContext.request.contextPath }/orderAdmin?method=getOrderDetail&orderId=${o.id}" class="FunctionButton">详细</a>

						<c:choose >
							<c:when test="${o.orderStatus==1}">
							<a href="${pageContext.request.contextPath }/orderAdmin?method=pay&tableId=${o.table_id}&orderId=${o.id}" class="FunctionButton">结账</a></td>
						</c:when>
						<c:otherwise>
...
<script>
	function toPage(page) {
        window.location.href="orderAdmin?method=list&page="+page;
    }
</script>
</body>
</html>
